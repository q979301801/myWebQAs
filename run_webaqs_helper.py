...
def define_common_squad_flags():
  ...
  flags.DEFINE_string('ouput_dir', None,
                      'The output dir that the result to save.')
  flags.DEFINE_bool(
      'do_lower_case', True,
...
def prediction_output_squad(strategy, input_meta_data, tokenizer, squad_lib,
  ...
  model_dir_path = os.path.abspath(FLAGS.model_dir)
  eval_writer = squad_lib.FeatureWriter(
      filename=os.path.join(model_dir_path, 'eval.tf_record'),
      is_training=False)
  eval_features = []
...
def dump_to_files(all_predictions,
  ...
  output_prediction_file = os.path.join(ouput_dir_path,
                                        '%spredictions.json' % file_prefix)
  output_nbest_file = os.path.join(ouput_dir_path,
                                   '%snbest_predictions.json' % file_prefix)
  output_null_log_odds_file = os.path.join(ouput_dir_path, file_prefix,
...
def eval_squad(strategy,
  ...
  if init_checkpoint is None:
    model_dir_path = os.path.abspath(FLAGS.model_dir)
    init_checkpoint = tf.train.latest_checkpoint(model_dir_path)

  predict_file_path = os.path.abspath(FLAGS.predict_file)
  all_predict_files = _get_matched_files(predict_file_path)
  if len(all_predict_files) != 1:
  ...
  with tf.io.gfile.GFile(FLAGS.predict_file, 'r') as reader:
    ...
    pred_dataset = dataset_json
  if input_meta_data.get('version_2_with_negative', False):
    eval_metrics = webqa_evaluate.evaluate(pred_dataset, all_predictions,
                                                scores_diff_json)
  else:
