...
def read_squad_examples(input_file, is_training,
  ...
  for k, v in input_data.items():
    # 获取问题文本
    question_txt = v['question']

    # 获取所有的问题ID，答案及段落内容
    for q_id, contents in v['evidences'].items():
      qid = q_id
      answer = contents['answer']
      content_value = contents['evidence']
      ...
      # 有一个答案并且答案无法在段落内找到完全匹配的内容，则过滤
      if (answer_txt not in ['no_answer', ''] and content.find(answer_txt) == -1):
        continue
      ...
      if is_training:
          # 有答案,则找出答案的位置
          if answer_txt.strip() not in ['no_answer', '']:
              start_position = content.find(answer_txt)
              end_position = len(answer_txt) + start_position - 1
              orig_answer_text = answer_txt

              # actual_text = "".join(content[start_position:(end_position +
              #                                                   1)])
              # # actual_text 为contxt的答案
              # # cleaned_answer_text 为自己写的答案
              # # actual_text要完全包括了cleaned_answer_text才行，可以比他长
...
def convert_examples_to_features(examples,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    tok_to_orig_index = [] #[0,0,1,1,1,2,3,3]  记录doc_tokens的每个子token ID
    orig_to_tok_index = [] #[0,2,5,6]          doc_tokens的第一个子token有两个，第二个子token有三个
    all_doc_tokens = []    #[a,b,c,d,e,f,g,h]  doc_tokens的全部子token
    ...
    doc_spans = []
    start_offset = 0
    # print('while start:-------------',example_index)
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      def process_paragraph(seg_p):
        for i in range(doc_span.length):
          ...
          # token_to_orig_map记录最终的tokens，每在all_doc_tokens中取一个词时对应的index，
          # 即tok_to_orig_index 例如token_to_orig_map[9:0,10:0,11:1,12:2]
          # 应该是用来最后拼接子token的
          token_to_orig_map[len(tokens)] = tok_to_orig_index[split_token_index]

          # 如果 max_tokens_for_doc < doc_stride 即spans为[0 -- max_tokens_for_doc]
          # [max_tokens_for_doc -- 2 * max_tokens_for_doc]这些区间永远不会重叠，
          # 所以每个位置split_token_index独一无二，is_max_context为True
          # 如果 max_tokens_for_doc > doc_stride 即spans为[0 -- max_tokens_for_doc]
          # [doc_stride -- doc_stride + max_tokens_for_doc]这些区间一定会有重叠部分，
          # 那么split_token_index的两个区间会有两个分数，取分数最高的那个区间，
          # 如果分数高的那个区间就是当前split_token_index的区间那么is_max_context为True，反之为Fales
          # 一般来说，区间长度越长，分数越高
...
def postprocess_output(all_examples,
  ...
  # 把每个example_id对应的feature写入字典,{exampelid1:[fea1,fea2],exampelid2:[fea3,fea4,fea5]}
    ...
    if not version_2_with_negative:
      ...
      if best_non_null_entry is not None:
        if xlnet_format:
          ...
          # 最高分比最低分还要小，或者不高于null_score_diff_threshold，那么无答案
  ...
  # all_predictions每个example一个最佳答案
  # all_nbest_json每个example的最佳20个答案，包含了逻辑值及概率值
  # scores_diff_json最好的答案和与无答案比较相差的逻辑分数值，小于0
