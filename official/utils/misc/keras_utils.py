...
my_global_batch_size_gauge = monitoring.IntGauge(
    '/tensorflow/training/my_global_batch_size', 'TF training global batch size')

my_first_batch_time_gauge = monitoring.IntGauge(
    '/tensorflow/training/my_first_batch',
    'TF training start/end time for first batch (unix epoch time in us.',
    'type')

first_batch_start_time = my_first_batch_time_gauge.get_cell('start')
first_batch_end_time = my_first_batch_time_gauge.get_cell('end')
...
class TimeHistory(tf.keras.callbacks.Callback):
  ...
  def __init__(self, batch_size, log_steps, initial_step=0, logdir=None):
    ...
    my_global_batch_size_gauge.get_cell().set(batch_size)
