...
# print(b1._nested_outputs) #返回全部的层输出 ，字典形式
# print(b1._nested_outputs['encoder_outputs']) #返回指定某个字典K的形式
# print(b1.outputs) #将字典去掉。返回全部层的输出
# super(BertEncoder, self).__init__(
#         inputs=[word_ids, mask, type_ids], outputs=outputs, **kwargs)
...
sequence_data = tf.keras.layers.Input(
        shape=(None, 100), name='sequence_data', dtype=tf.float32)
print('sequence_data',sequence_data)

sequence_data1 = tf.keras.layers.Input(
        shape=(100,20), name='sequence_data1', dtype=tf.float32)
print('sequence_data1',sequence_data1)
...
# start_logits, end_logits = _split_output_tensor(intermediate_logits)
...
sequence_data = tf.ones(shape=[5,6,10])

span_labeling = SpanLabeling(
        input_width=10,
    initializer='glorot_uniform',
    output='logits')
start_logits, end_logits = span_labeling.call(sequence_data)
print('start_logits',start_logits)
print('end_logits',end_logits)
...
j = tf.keras.layers.Lambda(
    tf.identity, name='start_positions1')
# print(start_logits.weights)