...
class MaskedLM(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    # self._vocab_size, self.hidden_size = self.embedding_table.shape
  ...
  def build(self, input_shape):
    ...
    self.bias = self.add_weight(
        ...
        initializer='zeros',
        trainable=True)
    # self.bias = tf.Variable(initial_value = tf.zeros(shape = [self._vocab_size,]),
    #                         shape=(self._vocab_size,),trainable=True)
  ...
  def call(self, sequence_data, masked_positions):
    ...
    # lm_data = tf.expand_dims(lm_data,axis=-1)
    ...
    logits1 = [masked_lm_input,lm_data,logits]

    if self._output_type == 'logits':
      return logits1,logits
    return logits1,tf.nn.log_softmax(logits)
