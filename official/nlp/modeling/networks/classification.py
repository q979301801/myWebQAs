...
class Classification(tf.keras.Model):
  ...
  *Note* that the network is constructed by
  ...
  def __init__(self,
    ...
    # weights = tf.Variable(initial_value = tf.zeros([num_classes,input_width])
    #             ,trainable=True,shape=[num_classes,input_width]
    #             ,dtype=tf.float32)
    #
    # logits = tf.matmul(cls_output, weights, transpose_b=True)
