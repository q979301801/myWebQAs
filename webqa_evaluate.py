...
def _make_qid_to_has_ans(dataset):
  ...
  for qas in dataset:
     # 每个问题ID都是否有答案
     qid_to_has_ans[qas['q_id']] = not qas['is_impossible']
...
def _compute_exact(a_gold, a_pred):
  a_preds = _normalize_answer(a_pred)
  a_golds = _normalize_answer(a_gold)
  print('a_preds: ',a_preds,'\t\t','a_golds: ',a_golds)
...
def _get_raw_scores(dataset, predictions):
  ...
  for qas in dataset:
    qid = qas['q_id']
    gold_answers = []
    answers = _normalize_answer(qas['answer_txt'])
    if answers is None:
      answers = ''
  ...
  for k, v in dataset.items():
    ...
    for q_id, contents in v['evidences'].items():
      if q_id in predictions:
        ...
        is_impossible = False
        if answer_txt  in ['no_answer', '']:
          answer_txt = ''
          is_impossible = True

        new_para = {'q_id':q_id,'question_txt':question_txt,'answer_txt':answer_txt,
                    'is_impossible':is_impossible}
        new_orig_data.append(new_para)
  # dataset：[{},{},{}...]
  dataset = new_orig_data

#  qa {"question": "What areas did Beyonce compete in when she was growing up?", "id": "56be85543aeaaa14008c9065",
#  "answers": [{"text": "singing and dancing", "answer_start": 207}], "is_impossible": false}
# 原文的dataset ：[ {'paragraphs':[{'qas': [qa]}] }...]
# dataset = new_orig_data
